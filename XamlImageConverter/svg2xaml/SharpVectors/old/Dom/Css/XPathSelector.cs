...
namespace SharpVectors.Dom.Css
{
	...
	public class XPathSelector
	{
        ...
        private static Regex reAttributeValueCheck = new Regex("^" + CssStyleRule.attributeValueCheck + "?$", RegexOptions.Compiled);
        ...
        private static Regex reLang = new Regex(@"^lang\(([A-Za-z\-]+)\)$", RegexOptions.Compiled);
        private static Regex reContains = new Regex("^contains\\((\"|\')?(?<stringvalue>.*?)(\"|\')?\\)$", RegexOptions.Compiled);
        private static Regex reNth = new Regex(@"^(?<type>(nth-child)|(nth-last-child)|(nth-of-type)|(nth-last-of-type))\(\s*(?<exp>(odd)|(even)|(((?<a>[\+-]?\d*)n)?(?<b>[\+-]?\d+)?))\s*\)$", RegexOptions.Compiled);
